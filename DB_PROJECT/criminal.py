...
class Criminal:
    ...
    def AddCRRecord(self,crid,crn,crage,crgndr,cradrs,crcellno,crstat,oid,fid):
        ...
        with closing(conn.cursor()) as c:
                insertQuery='insert into CRIMINAL(criminal_id,cr_name,age,gender,address,cell_no,status,officer_id,fir_id) values(?,?,?,?,?,?,?,?,?)'
                c.execute(insertQuery,(crid,crn,crage,crgndr,cradrs,crcellno,crstat,oid,fid))
                conn.commit()
                return '1'
